//! EPW001a / EPW007 infiltration diagnostic probes.
//!
//! These probes deliberately keep the EDR reference comparison non-gating until
//! the exact NTA 8800 §11.2.1.5 pressure-balance equations are implemented.
...
/// Diagnostic pressure-balance model.
///
/// The leakage normalization follows the identified NTA 8800 qv10 reference
/// chain. The path coefficients/pressure solver below are intentionally marked
/// diagnostic: the exact §11.2.1.5 step equations still need to be transcribed
/// from the controlled NTA source before this can become conformance code.
fn monthly_infiltration(
    climate: &nta8800_model::ClimateData,
    indoor_temperature_c: f64,
    qv10_spec_reken_dm3_s_m2: f64,
    ftype: f64,
    ...
    const AG_M2: f64 = 96.0;

    let qv10_lea_ref = qv10_spec_reken_dm3_s_m2 * ftype;
    let qv1_lea_ref = qv10_lea_ref * AG_M2 * 3.6 / 10.0_f64.powf(N_LEA);
    let paths = [
    ...
    MonthlyProfile::new(std::array::from_fn(|idx| {
        ...
        let mass_sum = |p_ref: f64| {
            let mut sum = rho_supply * mechanical_supply_m3_h - rho_i * mechanical_exhaust_m3_h;
            let mut q_in = 0.0;
            for (c, h, cp) in paths {
        ...
        let mut pa = (external_pressures.iter().copied().fold(f64::INFINITY, f64::min)
            + external_pressures.iter().copied().fold(f64::NEG_INFINITY, f64::max)) / 2.0
            + rho_i * H_WINDWARD * G;
        ...
        for _ in 0..200 {
            ...
            if mc.abs() <= ACCURACY_KG_PER_H { break; }
...
fn ventilation_result(
    ...
    let infiltration = monthly_infiltration(
        climate, indoor_temperature_c, qv10_spec_reken_dm3_s_m2, 1.4,
        MECHANICAL_FLOW_M3_H, MECHANICAL_FLOW_M3_H, WTW_EFF,
    );
...
fn calculate_qh_per_m2(
    zone: &Rekenzone,
    climate: &nta8800_model::ClimateData,
    transmission: &nta8800_transmission::TransmissionResult,
    qv10_spec_reken_dm3_s_m2: f64,
) -> (f64, f64) {
    let ventilation = ventilation_result(climate, 20.0, qv10_spec_reken_dm3_s_m2);
    ...
    (qh_kwh / zone.floor_area, ventilation.annual_q_v)
}
...
fn epw001a_infiltration_differential_probe() {
    ...
    let transmission = calculate_transmission(&zone, &transmission_elements(), &[], &[], &indoor, &climate, h_g_an, &HashMap::new(), &HashMap::new())
        .expect("EPW001a transmission calculation should succeed");

    let cases = [("EPW007c", 0.20, 37.98), ("EPW001a", 0.70, 42.69), ("EPW007a-input", 2.94, f64::NAN)];
    for (name, qv10, reference) in cases {
        let (qh_per_m2, qv_an) = calculate_qh_per_m2(&zone, &climate, &transmission, qv10);
        let error = if reference.is_finite() { (qh_per_m2 - reference).abs() / reference * 100.0 } else { f64::NAN };
        eprintln!("{name}: qv10_spec_reken={qv10:.3} dm³/(s·m²); Q_V;an={qv_an:.3} MJ; Q_H;nd;net={qh_per_m2:.6} kWh/m²; EDR={reference:.2}; error={error:.3}%");
    }

    // This is diagnostic only. A hard ±1% gate belongs here only after the
    // exact NTA pressure-balance equations have been transcribed and verified.
    let (baseline, _) = calculate_qh_per_m2(&zone, &climate, &transmission, 0.70);
